                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @forelse($cliente as $clienteItem)
                        ...
                        <td class="px-2 py-3 text-center">
                            <p class="font-semibold">{{$clienteItem->nit}}</p>
                        </td>
                        ...
                        <td class="flex px-2 py-3 text-sm text-center">
                            <a href="{{route('clientes.show',$clienteItem->nit)}}" class="ml-2 px-4 py-1 font-semibold leading-tight text-orange-700 bg-orange-100 rounded-full dark:bg-orange-500 dark:text-orange-100">Ver</a>
                            <a href="{{route('clientes.edit',$clienteItem->nit)}}" class="ml-2 px-4 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">Editar</a>
                            @if (Auth::check() && Auth()->user()->type == 1)
                                <form action="{{route('clientes.destroy',$clienteItem->nit)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
