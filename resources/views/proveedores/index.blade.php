...
@section('content')
    ...
    <div class="text-center mt-8 items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple">
        <h1 class="text-2xl">PROVEEDORES</h1> 
    </div>
              ...
              <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @forelse($proveedor as $proveedorItem)
                        <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-2 py-3 text-center">
                            <p class="font-semibold">{{$proveedorItem->nitProveedor}}</p>
                        </td>
                        <td class="px-4 py-3 text-sm text-center">
                            {{$proveedorItem->nombreProveedor}}
                        </td>
                        <td class="px-4 py-3 text-xs text-center">
                            {{$proveedorItem->telefono}}
                        </td>
                        <td class="px-4 py-3 text-sm text-center">
                            {{$proveedorItem->direccion}}
                        </td>
                        <td class="flex px-2 py-3 text-sm text-center">
                            <a href="{{route('proveedores.show',$proveedorItem->nitProveedor)}}" class="ml-2 px-4 py-1 font-semibold leading-tight text-orange-700 bg-orange-100 rounded-full dark:bg-orange-500 dark:text-orange-100">Ver</a>
                            <a href="{{route('proveedores.edit',$proveedorItem->nitProveedor)}}" class="ml-2 px-4 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">Editar</a>
                            @if (Auth::check() && Auth()->user()->type == 1)
                                <form action="{{route('proveedores.destroy',$proveedorItem->nitProveedor)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
